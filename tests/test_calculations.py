...
def test_add_calculation(setup_calculations):  # pylint: disable=redefined-outer-name, unused-argument
    """Test adding a calculation to the history."""
    calc = Calculation(Decimal('2'), Decimal('2'), add)
    Calculations.add_calculation(calc)
    assert Calculations.get_latest() == calc, "Failed to add the calculation to the history"

def test_get_history(setup_calculations):  # pylint: disable=redefined-outer-name, unused-argument
    """Test retrieving the entire calculation history."""
    history = Calculations.get_history()
    assert len(history) == 2, "History does not contain the expected number of calculations"

def test_clear_history(setup_calculations):  # pylint: disable=redefined-outer-name, unused-argument
    """Test clearing the entire calculation history."""
    Calculations.clear_history()
    assert len(Calculations.get_history()) == 0, "History was not cleared"

def test_get_latest(setup_calculations):  # pylint: disable=redefined-outer-name, unused-argument
...
def test_find_by_operation(setup_calculations):  # pylint: disable=redefined-outer-name, unused-argument
